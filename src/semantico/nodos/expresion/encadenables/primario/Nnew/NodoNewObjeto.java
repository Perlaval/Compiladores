...
public class NodoNewObjeto extends NodoNew {
    ...
    public Tipo chequear(TablaSimbolos ts) throws ErrorSemantico {
        ...
        if (listaParametros.size() != listaArgumentos.size()) throw new ErrorSemantico(token, "La cantidad de argumentos del constructor de la clase " + token.getLexema() + " no coincide");
        ...
        //2. Chequeamos que el tipo de los argumentos actuales coincida con la posicion y el tipo de los agumentos que recibe el constructor de la clase
        List<RegistroParametro> parametrosOrdenados = new ArrayList<>(listaParametros.values());
        parametrosOrdenados.sort(Comparator.comparingInt(RegistroParametro::getPos));

        int i = 0;
        for (RegistroParametro reg: parametrosOrdenados){
            NodoExpresion expresion = listaArgumentos.get(i);
        //for (NodoExpresion expresion: listaArgumentos) {
            ...
            if (reg.getTipo().esTipoReferencia()) {
                if (!ts.conforma(reg.getTipo().getNombreTipo(), expresion.chequear(ts).getNombreTipo()))
                    throw new ErrorSemantico(reg.tokenVarLocal, "El tipo de la variable " + expresion.getToken().getLexema() + " no coincide con el tipo esperado " + reg.getTipo());
